...
export default function AdminSubjects() {
  ...
  const [editingSubjectCode, setEditingSubjectCode] = useState<string | null>(
    null,
  );
  ...
  const availablePrograms = React.useMemo(() => {
    return adminRole === "superadmin"
      ...
      : programs
          .map((p) => ({
            ...p,
            departments: p.departments.filter(
              (d) =>
  ...
  React.useEffect(() => {
    ...
    const totalDepts = availablePrograms.reduce(
      (acc, p) => acc + p.departments.length,
      0,
    );
    if (totalDepts === 1) {
  ...
  const handleSubmitSubject = async (e: React.FormEvent) => {
    ...
    if (
      ...
      subjects.some((s) => s.code.toUpperCase() === code)
    ) {
    ...
    try {
      ...
      if (editingSubjectCode) {
        updatedSubjects = updatedSubjects.map((s) =>
          s.code === editingSubjectCode ? subjectData : s,
        );
      } else {
      ...
      setError(
        `Failed to ${editingSubjectCode ? "update" : "add"} subject: ` +
          err.message,
      );
    }
    setSaving(false);
  ...
  const exportSubjectsCsv = () => {
    ...
    const dataToExport =
      displayedSubjects.length > 0 ? displayedSubjects : subjects;
    ...
    link.setAttribute(
      "download",
      `Subjects_List_${new Date().toISOString().split("T")[0]}.csv`,
    );
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
        <div>
          ...
          <p className="mt-2 text-[11px] text-gray-500">
            Create and manage subjects, assign them to departments, year, and
            semester.
          </p>
        </div>
        <Button
          ...
          disabled={configLoading || displayedSubjects.length === 0}
        ...
        <div className="bg-indigo-50 px-4 py-3 flex items-center justify-between border-b border-indigo-100">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="text-sm font-bold uppercase tracking-wider text-indigo-900">
              {editingSubjectCode ? "Edit Subject" : "Add New Subject"}
            </h2>
          </div>
          {editingSubjectCode && (
            <Button
                      ...
                      {prog.departments.map((dept) => {
                        ...
                        return (
                          ...
                          >
                            <input
                              type="checkbox"
                              checked={newSubjectDepartments.includes(
                                deptValue,
                              )}
            ...
            <div className="flex justify-between items-center pt-2">
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="text-[11px] text-indigo-600 hover:text-indigo-800 underline flex items-center gap-1"
                >
              ...
              <Button
                ...
                disabled={
                  ...
                  !newSubjectCode.trim() ||
                  !newSubjectName.trim() ||
                  newSubjectDepartments.length === 0
                }
      ...
      <div className="bg-white rounded-xl shadow-md border border-gray-300 overflow-hidden">
        <div className="bg-indigo-50 px-4 py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-indigo-100">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="text-sm font-bold uppercase tracking-wider text-indigo-900">
              Existing Subjects ({displayedSubjects.length})
            </h2>
          </div>
          <div className="flex items-center gap-2">
            ...
            <table className="w-full text-left text-xs whitespace-nowrap">
              ...
              <tbody className="divide-y divide-gray-100">
                {displayedSubjects.map((sub, index) => (
                  <tr
                    key={sub.code}
                    className={`hover:bg-gray-50/50 transition-colors ${editingSubjectCode === sub.code ? "bg-indigo-50/30" : ""}`}
                  >
                        ...
                        {!sub.departments || sub.departments.length === 0 ? (
                          ...
                          sub.departments.map((d) => {
                            ...
                            return (
                              <span
                                key={d}
                                className="bg-gray-100 text-gray-700 px-2 py-0.5 rounded text-xs truncate max-w-[200px]"
                                title={displayLabel}
                              >
                                {displayLabel}
                              </span>

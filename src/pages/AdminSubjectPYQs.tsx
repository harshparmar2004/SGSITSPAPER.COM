...
export default function AdminSubjectPYQs() {
  ...
  // Grouping structure: Department -> Semester -> Subject -> PYQs
  ...
  // Group by Department
  const groupedByDepartment = filteredPyqs.reduce((acc, pyq) => {
  ...
  const organizeBySemester = (deptPyqs: PYQ[]) => {
    return deptPyqs.reduce((acc, pyq) => {
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {Object.keys(groupedByDepartment).sort((a,b) => a.localeCompare(b)).map(departmentKey => {
            ...
            const isDeptExpanded = expandedDepartment === departmentKey || (search && true); 
              ...
              <div key={departmentKey} className="bg-white border text-gray-900 border-gray-200 rounded-xl overflow-hidden shadow-sm">
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-semibold text-gray-900 text-xl">{departmentKey}</span>
                    <span className="ml-2 text-xs font-medium bg-indigo-100 text-indigo-800 px-2 py-0.5 rounded-full">{deptGroup.length} PDFs</span>
                  </div>
                  {isDeptExpanded ? <ChevronDown className="w-5 h-5 text-gray-600" /> : <ChevronRight className="w-5 h-5 text-gray-600" />}
                </button>

                {isDeptExpanded && (
                  <div className="bg-white flex flex-col gap-[2px] p-2">
                     {Object.keys(semestersObj).sort((a,b) => a.localeCompare(b)).map(semesterKey => {
                       const semGroup = semestersObj[semesterKey];
                       const subjectsObj = organizeBySubject(semGroup);
                       const isSemExpanded = expandedSemester === semesterKey || (search && true);

                       return (
                         <div key={semesterKey} className="border border-gray-100 rounded-lg overflow-hidden mb-2 last:mb-0">
                            <button
                               onClick={() => setExpandedSemester(isSemExpanded ? null : semesterKey)}
                               className="w-full flex items-center justify-between p-3 bg-gray-50 hover:bg-gray-100 transition-colors"
                            >
                               <div className="flex items-center gap-2">
                                 <BookOpen className="w-5 h-5 text-indigo-500" />
                                 <span className="font-medium text-gray-800 text-lg">{semesterKey}</span>
                                 <span className="ml-2 text-xs font-medium bg-gray-200 text-gray-700 px-2 py-0.5 rounded-full">{semGroup.length} PDFs</span>
                               </div>
                               {isSemExpanded ? <ChevronDown className="w-4 h-4 text-gray-500" /> : <ChevronRight className="w-4 h-4 text-gray-500" />}
                            </button>
                            ...
                            {isSemExpanded && (
                               <div className="bg-white p-2">
                                  {Object.keys(subjectsObj).sort((a,b) => a.localeCompare(b)).map(subjectKey => {
                                    ...
                                    return (
                                      <div key={subjectKey} className="border border-gray-100 rounded-lg overflow-hidden mb-2 last:mb-0">
                                         <button

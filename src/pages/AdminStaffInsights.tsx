...
import { Loader2, Download, UserCircle, X, Activity, FileText } from "lucide-react";
...
export default function AdminStaffInsights() {
  ...
  const [selectedStaffInsight, setSelectedStaffInsight] = useState<any | null>(null);
  ...
  const fetchAnalytics = async () => {
    ...
    try {
      ...
      const staffMembers = adminList.filter((a: any) => a.role === "department");
      ...
      staffMembers.forEach((staff: any) => {
        if (staff.email) {
          staffMap[staff.email.toLowerCase()] = {
            ...
            name: staff.name || uidToName.get(staff.id) || staff.email.split("@")[0],
      ...
      allPyqs.forEach((doc: any) => {
        ...
        const mappedStaffKey = staffMap[staffKey] ? staffKey : (uidToEmail.get(doc.uploadedBy) || "");

        const staffRef = staffMap[mappedStaffKey] || Object.values(staffMap).find((s:any) => s.id === doc.uploadedBy);

        if (staffRef && doc.uploadedAt) {
          const dStr = format(new Date(doc.uploadedAt.seconds * 1000), "MMM yyyy");
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  {selectedStaffInsight.email} &middot; Activity History
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
              {Object.values(staffInsights).map((staff: any) => (
                <div
                  ...
                  className="p-3 bg-gray-50/80 hover:bg-indigo-50 border border-gray-100 rounded-lg cursor-pointer transition-colors shadow-sm text-center flex flex-col"
                >
                  ...
                  <p className="text-[9px] text-gray-500 line-clamp-1 mt-0.5 mb-2" title={staff.departments}>
                     {staff.departments}
                  </p>
